...
locals {
  stgaccname = format("${var.stgaccname}${var.company}${var.env}${random_string.this.result}")
}
...
resource "azurerm_databricks_workspace" "dbwdata01" {
  name                = format("dbwdata-${var.company}${var.env}${random_string.this.result}")
  resource_group_name      = var.resource_group_name
  location                 = var.location
  sku                 = "standard"
...
resource "azurerm_key_vault" "kvdatabricks" {
  name                        = format("akv-${var.company}${var.env}${random_string.this.result}") //update the keyvault name
  location                    = var.location
  resource_group_name         = var.resource_group_name
...
resource "databricks_cluster" "dbcluster01" {
  ...
  cluster_name            = format("dbcluster-${var.stgaccname}${var.company}${var.env}${random_string.this.result}")
  num_workers             = 0
